...
| **LLM Engine** | Gemini (`gemini-3.1-flash-lite`) | High-performance reasoning on a generous free-tier |
...
To maximize responsiveness while keeping API costs low, requests are routed based on turn characteristics (both default to `gemini-3.1-flash-lite` for rate-limit stability, but can be customized to any model including `gemini-3.5-flash` or `gemini-3.1-pro`):

| Scenario | Model Routed | Rationale |
| :--- | :--- | :--- |
| **Early conversation / vague query** | `gemini-3.1-flash-lite` | Ultra-fast responses to clarify customer needs |
| **Shortlisting / complex refinements** | `gemini-3.1-flash-lite` | Deep logical reasoning to select precise metrics |
| **Near turn budget limit (Turn ≥ 5)** | `gemini-3.1-flash-lite` | High precision to finalize recommendations in time |
| **Detailed job description / long input** | `gemini-3.1-flash-lite` | Large context processing to match job competencies |
...
| `MODEL_POWERFUL` | No | `gemini-3.1-flash-lite` | Model for recommendations, reasoning, and finalizations |
| `MODEL_LIGHT` | No | `gemini-3.1-flash-lite` | Model for intent classification and early clarifications |
